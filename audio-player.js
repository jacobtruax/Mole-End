class MediaPlayer {
	...
	render(options) {
		...
		this.containerAudio = this.createElement('div', {
			class: 'playr'
		});

		this.player.parentNode.insertBefore(this.containerAudio, this.player);
		this.containerAudio.appendChild(this.player);
	...
	setDimensions() {
		var _scope = this;
		_scope.uiRect = _scope.containerAudio.getBoundingClientRect();
		_scope.barRect = _scope.progressBar.getBoundingClientRect();

		if (_scope.uiRect.width < 620) {
			_scope.containerAudio.classList.add('compact');
		} else {
			_scope.containerAudio.classList.remove('compact');
		}
	...
	play() {
		...
		if (!!this.player.paused) {
			this.containerAudio.classList.add('playing');
			this.containerAudio.classList.remove('paused');
			this.player.play();
		} else {
			this.pause();
		}
	...
	pause() {
		...
		this.containerAudio.classList.add('paused');
		this.containerAudio.classList.remove('playing');
		this.player.pause();
	}
	...
	createUI() {
		...
		this.containerAudio.appendChild(ui);
